...
class TradingApiPosition extends TradingApi
{
    use HasFactory;

    // public function getOpenPositions(Request $request){
    //     return $this->getPositions($request, 'getOpenPositions');
    // }

    public function getClosedPositions($account_id){
        ...
        if(array_key_exists('data', $json))
        {
            foreach ($json->data  as $position ) {
                ...
                $tradingPosition->fill(( array )$position);
    ...
    public function getPositions($uri){

        $url="https://www.trade-copier.com/webservice/v4/position/".$uri.".php";
        $headers =  array(
            'Content-Type: application/x-www-form-urlencoded',
            'Auth-Username: '. $this->username,
            'Auth-Token: '. $this->auth_token,
