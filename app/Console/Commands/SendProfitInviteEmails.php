...
class SendProfitInviteEmails extends Command
{
    protected $signature = 'email:invite';
    protected $description = 'Send invitation notifications to users based on yearly summed PnL';
    ...
    public function handle()
    {
        ...
        $this->info("============== SendProfitInviteEmails START : {$modifiedUsers->count()} ==============");
        ...
        foreach ($modifiedUsers as $apiReport) {
            try {
                ...
                if ($user->email_subscribed) {
                    ...
                    $rules = NotificationRule::where('type', 'invite')->get();
                    ...
                    foreach ($rules as $rule) {
                        ...
                        // Check min_value, max_value, and vip_level for the rule
                        if (($rule->min_value === null || $annualPnL >= $rule->min_value) &&
                            ($rule->max_value === null || $annualPnL < $rule->max_value) &&
                            ($rule->risk_level === null || $this->matchesRiskLevel($userRiskLevel, $rule->risk_level))) {
                            ...
                            if (class_exists($notificationClass)) {
                                ...
                                Log::channel('command')->info("email:invite => Notification sent to: {$user->email} with annual PnL: {$annualPnL} using {$notificationClass}");
                                $sent_amount += 1;
                            } else {
                ...
                // sleep(5); // Uncomment if you need a delay between notifications
            } catch (\Throwable $th) {
        ...
        $this->info("============== SendProfitInviteEmails END : {$sent_amount} ==============");
    }
